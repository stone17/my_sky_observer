...
def get_cache_info(object_name: str, setup_hash: str):
    ...
    sanitized_name = (
        sanitized_name.replace(" ", "_").replace("°", "d").replace("'", "m")
    )
...
async def download_image(
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            ...
            stretched_bytes = await asyncio.to_thread(
                auto_stretch_image, response.content
            )
...
def get_sorted_objects(
    settings: dict, location: Location, telescope: Telescope, camera: Camera
) -> List[dict]:
    ...
    try:
        session_start, session_end = calculator.get_observing_session(
            observer, Time.now()
        )
        altaz_frame, night_duration = calculator.prepare_night_frame(
            location, session_start, session_end
        )
    except Exception:
    ...
    if altaz_frame is not None:
        ...
        df["hours_visible"] = calculator.batch_calculate_nightly_hours(
            all_ras, all_decs, altaz_frame, night_duration, min_alt
        )
    else:
    ...
    def sort_func(x):
        ...
        for k in sort_keys:
            if k == "brightness":
                res.append(x.get("magnitude", 99))
            elif k == "size":
                res.append(-(x.get("size") or 0))
            elif k == "time" or k == "altitude":
                res.append(-(x.get("max_altitude") or 0))
            elif k == "hours_above":
                res.append(-(x.get("hours_visible") or 0))
        return tuple(res)
...
class StreamSession:
    def __init__(self, request: Request, settings: dict):
        ...
        self.setup_hash = get_setup_hash(
            self.fov_w_deg,
            self.fov_h_deg,
            self.img_padding,
            self.img_res,
        ...
        observer = Observer(
            location=EarthLocation(
                lat=self.location.latitude * u.deg, lon=self.location.longitude * u.deg
            )
        )
        self.session_start, self.session_end = calculator.get_observing_session(
            observer, Time.now()
        )
        self.twilight = calculator.get_twilight_periods(
            self.location, self.session_start
        )
    ...
    def get_initial_metadata_event(self):
        ...
        for obj in self.all_objects:
            ...
            if obj.get("min_ax", 0) > 0:
                min_ax = obj["min_ax"]
                if isinstance(min_ax, (int, float)):
                    min_ax = round(min_ax, 2)
                size_str += f" x {min_ax}'"
    ...
    def prioritize_top_objects(self):
        ...
        max_mag = self.settings.get(
            "max_magnitude", client_settings.get("max_magnitude", 12)
        )
        min_size = self.settings.get("min_size", client_settings.get("min_size", 0))
        sel_types = self.settings.get(
            "selected_types", client_settings.get("selected_types", [])
        )
        min_hrs = self.settings.get("min_hours", client_settings.get("min_hours", 0))
        min_alt = self.settings.get("min_altitude", 30.0)

        filter_active_alt = self.settings.get(
            "filter_active_alt", client_settings.get("filter_active_alt", True)
        )
        filter_na_alt = self.settings.get(
            "filter_na_alt", client_settings.get("filter_na_alt", True)
        )
        filter_active_mag = self.settings.get(
            "filter_active_mag", client_settings.get("filter_active_mag", True)
        )
        filter_na_mag = self.settings.get(
            "filter_na_mag", client_settings.get("filter_na_mag", True)
        )
        filter_active_size = self.settings.get(
            "filter_active_size", client_settings.get("filter_active_size", True)
        )
        filter_na_size = self.settings.get(
            "filter_na_size", client_settings.get("filter_na_size", True)
        )
        filter_active_hours = self.settings.get(
            "filter_active_hours", client_settings.get("filter_active_hours", True)
        )
        filter_na_hours = self.settings.get(
            "filter_na_hours", client_settings.get("filter_na_hours", True)
        )

        sel_types = [t.lower().strip() for t in sel_types]

        def check_obj(o):
            if filter_active_mag:
                mag = o.get("magnitude", 99)
                is_na = mag >= 99
                if is_na and not filter_na_mag:
                    return False
                elif not is_na and mag > max_mag:
                    return False

            if filter_active_hours:
                hrs = o.get("hours_visible")
                is_na = hrs is None
                hrs = hrs or 0
                if is_na and not filter_na_hours:
                    return False
                elif not is_na and hrs < min_hrs:
                    return False

            if filter_active_size:
                size = o.get("maj_ax", 0)
                is_na = size == 0
                if is_na and not filter_na_size:
                    return False
                elif not is_na and size < min_size:
                    return False

            if filter_active_alt:
                alt = o.get("max_altitude")
                is_na = alt is None
                alt = alt or 0
                if is_na and not filter_na_alt:
                    return False
                elif not is_na and alt < min_alt:
                    return False

            if sel_types:
                o_type = str(o.get("type", "")).strip().lower()
                if o_type not in sel_types:
                    return False
            return True
    ...
    async def stream_details(self):
        ...
        for obj in self.top_objects:
            ...
            status = (
                "cached"
                if is_cached
                else ("queued" if obj_id in download_ids else "pending")
            )
        ...
        async def worker(obj):
            ...
            async with semaphore:
                ...
                try:
                    ...
                    completed += 1
                    await queue.put(
                        {"progress": True, "current": completed, "total": total}
                    )
...
async def geocode_city(city: str):
    if not city or len(city) < 2:
        return JSONResponse(
            content={"error": "City name is too short"}, status_code=400
        )
        ...
        results = [
            {
                ...
                "admin1": r.get("admin1", ""),
                "latitude": round(r.get("latitude"), 4),
                "longitude": round(r.get("longitude"), 4),
            }
...
async def stream_objects_endpoint(
    request: Request,
    focal_length: float,
    sensor_width: float,
    sensor_height: float,
    ...
    if "min_altitude" in qp:
        settings["min_altitude"] = float(qp["min_altitude"])
    if "min_hours" in qp:
        settings["min_hours"] = float(qp["min_hours"])
    if "image_resolution" in qp:
        settings["image_resolution"] = int(qp["image_resolution"])
    if "max_magnitude" in qp:
        settings["max_magnitude"] = float(qp["max_magnitude"])
    if "min_size" in qp:
        settings["min_size"] = float(qp["min_size"])
    if "image_padding" in qp:
        settings["image_padding"] = float(qp["image_padding"])
    if "image_timeout" in qp:
        settings["image_timeout"] = int(qp["image_timeout"])
    if "download_mode" in qp:
        settings["download_mode"] = qp["download_mode"]
    if "image_source" in qp:
        settings["image_source"] = qp["image_source"]

    if "filter_active_alt" in qp:
        settings["filter_active_alt"] = qp["filter_active_alt"].lower() == "true"
    if "filter_na_alt" in qp:
        settings["filter_na_alt"] = qp["filter_na_alt"].lower() == "true"
    if "filter_active_hours" in qp:
        settings["filter_active_hours"] = qp["filter_active_hours"].lower() == "true"
    if "filter_na_hours" in qp:
        settings["filter_na_hours"] = qp["filter_na_hours"].lower() == "true"
    if "filter_active_mag" in qp:
        settings["filter_active_mag"] = qp["filter_active_mag"].lower() == "true"
    if "filter_na_mag" in qp:
        settings["filter_na_mag"] = qp["filter_na_mag"].lower() == "true"
    if "filter_active_size" in qp:
        settings["filter_active_size"] = qp["filter_active_size"].lower() == "true"
    if "filter_na_size" in qp:
        settings["filter_na_size"] = qp["filter_na_size"].lower() == "true"
...
async def get_object_details(obj_id: str):
    ...
    camera = Camera(
        **settings.get("camera", {"sensor_width": 23.5, "sensor_height": 15.6})
    )
    ...
    setup_hash = get_setup_hash(
        fov_w,
        fov_h,
        padding,
        img_srv.get("resolution", 512),
        img_srv.get("source", "dss2r"),
    )
...
async def download_object_endpoint(request: Request):
    try:
        ...
        url = await download_image(
            ...
            obj["name"],
            setup_hash,
            resolution=img_srv.get("resolution", 512),
            source=img_srv.get("source", "dss2r"),
            timeout=img_srv.get("timeout", 60),
        )
...
async def send_to_nina(request: NinaFramingRequest):
    ...
    try:
        if isinstance(request.ra, (float, int)) and isinstance(
            request.dec, (float, int)
        ):
        ...
        async with httpx.AsyncClient() as client:
            ...
            resp_coord = await client.get(
                coord_url,
                params={"RAangle": coords.ra.deg, "DecAngle": coords.dec.deg},
                timeout=2.0,
            )
            if resp_coord.status_code >= 400:
                raise HTTPException(
                    status_code=502,
                    detail=f"Coordinate error: {resp_coord.status_code}",
                )

            # 2. Send Rotation
            resp_rot = await client.get(
                rot_url, params={"rotation": -request.rotation}, timeout=2.0
            )
            if resp_rot.status_code >= 400:
                raise HTTPException(
                    status_code=502, detail=f"Rotation error: {resp_rot.status_code}"
                )
...
async def get_cached_image(setup_hash: str, image_filename: str):
    ...
    return FileResponse(
        filepath, headers={"Cache-Control": "no-cache, no-store, must-revalidate"}
    )


static_relative = (
    "frontend/dist"
    if hasattr(sys, "_MEIPASS") or os.path.exists("frontend/dist")
    else "frontend"
)
